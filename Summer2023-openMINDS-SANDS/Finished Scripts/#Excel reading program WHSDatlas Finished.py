...
import copy
#Set the file path
Hierarchy = pd.read_excel(r'C:\Users\mats_\Desktop\Ebrains\Visual Code\WHS SD rat brain atlas v4 labels for MBAT Adjusted Cortex.xlsx')
...
#Remember to adjust information on excel to not skip anything, empty columns between regions

#Sets up the data frame and the dictonary for storing hierarchy data
SimplifiedHierarchy = {'id':[], 'name':[], 'hierarchy_level':[], 'abbreviation' : []}
...
for index, row in Hierarchy.iterrows():
    ...
    hierarchyColumns = row[1:-1]
    hierarchyIsNA= np.where(~hierarchyColumns.isna())[0]
    regionId = hierarchyColumns[hierarchyIsNA[0]]
    regionName = hierarchyColumns[hierarchyIsNA[1]]
    regionAbbreviation = hierarchyColumns[hierarchyIsNA[-1]]
    ...
    SHTemp = {'id':[regionId], 'name':[regionName], 'hierarchy_level':[hierarchyLevel], 'abbreviation' : regionAbbreviation}
    ...
    StoredHier |= {int(SimplifiedHierarchy.iloc[-1,2]) : [SimplifiedHierarchy.iloc[-1,2],SimplifiedHierarchy.iloc[-1,0],SimplifiedHierarchy.iloc[-1,1], SimplifiedHierarchy.iloc[-1,3]]}
    ...
    if NoParent == 1:
        SimplifiedHierarchy.loc[LastObject, "parent"] = None
        SimplifiedHierarchy.loc[LastObject, "parentID"] = None
...
#Saves it to excel file
SimplifiedHierarchy.to_excel(path, header= ["ID", "Name", "Hierarchy Level", "Abbreviation", "ParentName", "ParentID ", "HierarchyID"], index=False)
